...
长文（多 `##` 节、带表格和图）先做结构再抠句子：跨章节同义清单只留最全一份、表格旁正文别复读、判断/取舍类列表默认转 prose，详见 `write-zh.md` 的「结构级重复与表格复读（长文专项）」，整体流程见 `references/mode-long-form.md`。
