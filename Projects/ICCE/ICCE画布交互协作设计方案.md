...
本文说明画布协作的当前边界、协议设计和后续能力预案。当前实现以“当前实现基线”为准；实施阶段和目标增强仅作为规划依据。

## 阅读说明

本文同时包含当前实现基线、首期方案和后续能力预案。判断功能是否已经完成时，以“当前实现基线”和各章节中的当前状态为准；“目标增强”“后续总体能力”和“实施阶段”属于规划内容。

## 当前有效结论

1. 选中快照、过程预览和正式提交已经分开承载；过程态不写正式 Store，也不表达业务回滚。
2. 图形绘制、尺寸调整、移动和连线连接的首期过程协作已接入，最终结果仍由应用命令产生。
3. 冲突安全、服务端确认、超大选区降级和长期操作审计属于后续能力，不纳入首期完成判定。
...
区域 resize 已与普通图形复用同一过程协议和远端预览；区域绘制、`selectionRef` 和超大选区优化仍不属于当前实现范围。缩放本地过程已改为快照规划和纯预览，完整链路见 [[ICCE图形缩放预览计划与单次提交方案]]。

## 3. 当前实现基线

本节同时保留优化前问题基线与已经闭环的当前能力。3.1 及 3.4～3.8 描述当前状态；3.2～3.3 描述推动本方案形成的优化前问题，不再作为当前实现事实。
...
以下小节分别标明“当前状态”或“优化前问题”，不能将历史问题重新解释为当前实现。
...
### 3.2 优化前问题：实时变更数据量随选区规模增长

- 实时消息默认按 16ms 节流，频率接近 60FPS。
- 多选拖拽每帧为每个选中图形生成绝对坐标更新。
- 协作编码为每个图形附带完整 `data`、尺寸和层级数据。
- 接收端曾把 live 数据直接写入正式 Store，引发响应式计算和正式渲染；当前过程态已迁移到独立预览状态，不再沿用该行为。
...
### 3.5 图形与区域 resize 过程协作已闭环

- 区域和普通图形共用 `shape-resize` 过程协议、缩放交互状态及提交和异常清理流程；区域保留子图形边界和区域重叠约束，普通图形保留父区域扩容与包含关系刷新。
- 本地 resize 过程基于手势开始时的几何快照生成纯计划，只更新本地几何预览，不逐帧写入正式图形 Store。
- 远端只接收轻量包围盒并更新独立预览 Store，不触发正式图形回放。
- 最终尺寸、区域扩容、后代联动和关联连线变化由应用层一次性执行完整计划，并通过正式操作同步广播；操作序号、基础版本和同目标冲突约束仍待补充。
- 缩放已与移动统一采用“快照规划、纯预览、单次提交”；完整区域联动的实时远端投影仍是后续增强项。
- 远端区域扩容与本地移动交错时，活动会话可能继续使用旧几何快照，导致区域推挤结果被陈旧计划覆盖；并发重基、`before` 几何校验和服务端基础版本校验属于待实施的冲突安全增强，完整方案见 [[ICCE图形缩放预览计划与单次提交方案]]。
...
- 本地 resize 过程基于初始几何快照生成计划，只更新本地几何预览；正式协作链路不发送逐帧的完整图形变更。
- resize 完成后继续通过现有应用命令产生 canonical commit；过程态 `commit` 负责收尾并清理远端预览。
- 目标失效、会话异常、页面卸载或主动中止时执行 `reset`。
- 区域和普通图形使用相同过程协议；区域尺寸约束仍由区域规则计算，远端不重复推导子图形和区域关系。
...
当前区域和普通图形已统一使用本节过程协议及纯计划预览链路。这里记录已落地的协作边界；缩放实现细节和区域语义见 [[ICCE图形缩放预览计划与单次提交方案]]。
...
本地 resize 过程只在快照上规划，并将计划投影到本地几何预览；因此不会产生逐帧的正式 `OPERATION_SYNC` 变更，也不需要依靠协作层过滤缩放 live 写入。过程消息仍通过 `shape-resize/update` 发送主目标的轻量边界，避免远端正式 Store 被过程帧改写。
...
- 过程态 commit 只携带会话编号，用于远端会话收尾；正式图形、区域、后代和连线仍以 canonical 消息为准。
...
详情页因角色切换等原因按路由键整体替换时，Vue 可能先创建新页面实例，再卸载旧页面实例。若模块级协作订阅由某个页面实例直接持有和清理，新页面会先复用旧页面已经建立的订阅，随后旧页面卸载又会把这组共享订阅全部销毁。此时 WebSocket 仍可正常离开并重新加入房间，但正式操作、过程交互和选中快照的本地发布链路已经失效，因此不能只用加入房间成功来判断协作链路完整。

`useCollabSync` 的共享运行时遵循以下生命周期约束：

1. 模块级事件订阅、远端消息订阅和选中状态监听由独立的响应式作用域持有，不归属于首次调用它的组件作用域，避免旧组件卸载时由 Vue 自动停止仍在复用的监听。
2. 每个页面实例初始化时取得唯一的页面持有凭证，卸载时只释放自己的凭证；新旧页面重叠期间继续复用同一组共享运行时。
3. 只有最后一个页面持有凭证释放后，才统一停止响应式作用域，并清理事件订阅、远端消息订阅、节流任务、消息分发器、远端状态和抑制状态。
4. 初始化失败时立即释放本次取得的凭证；单个页面的释放函数保持幂等，避免重复卸载导致引用关系失真。
5. 共享运行时始终至多保留一组，避免页面重叠期间重复订阅和重复发送消息。

这里的“页面持有凭证”只表示客户端共享资源的所有权，不是服务端用于并发控制的 transform 编辑租约，也不改变后续服务端冲突控制方案。

独立响应式作用域的实现约束：

- `effectScope(true)` 创建脱离当前组件作用域的响应式作用域；只有在 `scope.run()` 回调内创建的 `watch`、`watchEffect` 和 `computed` 等响应式副作用，才会被该作用域统一收纳。
- `scope.stop()` 只负责停止作用域内的 Vue 响应式副作用，不会自动取消 WebSocket、事件总线、定时器、节流器或远端 Store，因此这些资源仍必须由运行时清理函数显式释放。
- 独立作用域不是永久全局监听。共享运行时必须在最后一个页面持有凭证释放时调用 `scope.stop()`；如果遗漏释放，会导致监听和状态长期存活，形成重复发送或内存泄漏。
- 页面私有的响应式副作用仍应留在组件作用域中，只有需要跨页面实例复用且由共享运行时统一管理的副作用才使用脱离作用域。
...
- 路由键替换时新页面先创建、旧页面后卸载，旧页面释放后正式操作消息仍可发送。
- 页面重叠期间选中状态监听保持唯一且持续发送快照，最后一个页面卸载后才停止独立响应式作用域。
- 空选择清理。
- 大 ID 分片和哈希失败。
- 租约拒绝、超时和断线释放。
- 1千、1万、5万对象的性能基准。
